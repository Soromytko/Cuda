...
void calcSimple(const mat *mat1, const mat *mat2, const result_mat *resultMat)
{
    ...
    const int resultCount = s_gridDim.x * s_gridDim.y;
    ...
    calc_zero_count_simple<<<s_gridDim, s_threadDim>>>(dev_mat1, dev_mat2, dev_rawResultMat, MAT_DIM_X, MAT_DIM_Y);
    ...
    result_mat *cudaRawResultMat = new result_mat[matSize];
    CHECK(cudaMemcpy(cudaRawResultMat, dev_rawResultMat, matSize, cudaMemcpyDeviceToHost));
    ...
    assert(isMatEqual(resultMat, result));
...
void calcWithAtomic(const mat *mat1, const mat *mat2, const result_mat *resultMat)
{
    ...
    assert(isMatEqual(resultMat, cudaResult));
